...
class ApprovedServisMesin extends StatefulWidget {
  final String? idServisMesin;

  const ApprovedServisMesin({
    Key? key,
    this.idServisMesin,
  }) : super(key: key);

  @override
  ApprovedServisMesinState createState() => ApprovedServisMesinState();
}

class ApprovedServisMesinState extends State<ApprovedServisMesin> {
  ...
  Widget getTitle() {
    return Container(
      ...
      child: const Column(
        children: [
          Text(
            'APPROVED Setting Machine For Machine Maintenance',
  ...
  _simpanApprove() async {
    final res = await servisMesinBloc.finishServisMesin(widget.idServisMesin!);
